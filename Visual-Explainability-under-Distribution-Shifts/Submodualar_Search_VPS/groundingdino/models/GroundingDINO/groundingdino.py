...
class GroundingDINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets: List = None, **kw):
        ...
        # --- Query embeddings ---
        if hasattr(self.transformer, "tgt_embed"):
            num_queries = self.transformer.tgt_embed.num_embeddings
            query_embed_weight = self.transformer.tgt_embed.weight
        else:
            raise AttributeError("Could not find query embeddings (expected transformer.tgt_embed)")

        # --- Run transformer ---
        input_query_bbox = input_query_label = attn_mask = dn_meta = None

        # expand pos embeddings to match query count
        query_pos = query_embed_weight.unsqueeze(1).repeat(1, samples.tensors.shape[0], 1)  # [num_queries, batch, d_model]
