    ...
    def evaluation_maximun_sample(self, S_set):
        ...
        with torch.no_grad():
            ...
            if self.mode == "cls":
                ...
                max_values, _ = torch.max(logits, dim=2, keepdim=True)
                mask = max_values > self.threshold
                logits_clip = logits * mask
                ious_clip = ious
            
            cls_score = logits[:,:,self.target_label].max(dim=-1)[0]   # torch.Size([170, 900])
            cls_score_clip = logits_clip[:,:,self.target_label].max(dim=-1)[0]   # torch.Size([170, 900])
            
            insertion_scores = (ious_clip * cls_score).max(dim=-1)[0]
            insertion_scores_clip = (ious_clip * cls_score_clip).max(dim=-1)[0]
            ...
            if self.mode == "cls":
                ...
                max_values, _ = torch.max(logits_reverse, dim=2, keepdim=True)
                mask = max_values > self.threshold
                logits_reverse_clip = logits_reverse * mask
                ious_reverse_clip = ious_reverse
            
            cls_score_reverse = logits_reverse[:,:,self.target_label].max(dim=-1)[0]   # torch.Size([170, 900])
            cls_score_reverse_clip = logits_reverse_clip[:,:,self.target_label].max(dim=-1)[0]   # torch.Size([170, 900])
            
            deletion_scores = (ious_reverse_clip * cls_score_reverse).max(dim=-1)[0]
            deletion_scores_clip = (ious_reverse_clip * cls_score_reverse_clip).max(dim=-1)[0]
            ...
            smdl_scores = self.lambda1 * insertion_scores_clip + self.lambda2 * (1-deletion_scores_clip)
